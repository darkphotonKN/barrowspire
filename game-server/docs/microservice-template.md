...
| 服務 | Port | 來源 |
|------|------|------|
| api-gateway | 7114 | `PORT` |
| item-service | 7002 | `GRPC_ITEM_SERVICE_ADDR` |
| auth-service | 7116 | `GRPC_AUTH_ADDR` |
| game-service | 7117 | `GRPC_GAME_ADDR` |
| example-service | 7123 | `GRPC_EXAMPLE_ADDR` |
| stats-service | 7124 | `GRPC_STATS_ADDR` |
| auth-service (alt) | 7020 | `PORT` in auth-service/.env |
| payment-service | 7134 | `GRPC_PAYMENT_ADDR` |
...
| 服務 | Port | 來源 |
|------|------|------|
| game-service | 5143 | docker-compose |
| api-gateway | 5214 | docker-compose |
| auth-service | 5216 | docker-compose |
| stats-service | 5217 | docker-compose |
| example-service | 5223 | docker-compose |
| payment-service | 5224 | docker-compose |
...
```go
import (
	pb "github.com/darkphotonKN/barrowspire-server/common/api/proto/{service}"
...
import (
	...
	"github.com/darkphotonKN/barrowspire-server/common/discovery/consul"
	commonhelpers "github.com/darkphotonKN/barrowspire-server/common/utils"
	"github.com/darkphotonKN/barrowspire-server/{service-name}/config"
	"github.com/darkphotonKN/barrowspire-server/{service-name}/internal/{service}"
...
DB_NAME=barrowspire_{service}_service_db

# gRPC
GRPC_{SERVICE_UPPER}_ADDR={GRPC_PORT}

# RabbitMQ
RABBITMQ_USER=barrowspire
RABBITMQ_PASS=barrowspire
RABBITMQ_HOST=localhost
RABBITMQ_PORT=5795

# Service Discovery
CONSUL_ADDR=localhost:8623
```
...
services:
  db:
    image: postgres:17
    container_name: barrowspire_{service}_service_db
    environment:
      POSTGRES_USER: user
      POSTGRES_PASSWORD: password
      POSTGRES_DB: barrowspire_{service}_service_db
    ports:
      - "{DB_PORT}:5432"
    volumes:
...
networks:
  barrowspire-network:
    external: true
    name: barrowspire-network
```

### 9. `go.mod`

```
module github.com/darkphotonKN/barrowspire-server/{service-name}
...
migrate-up:
	@migrate -path ./migrations -database "postgres://user:password@localhost:{DB_PORT}/barrowspire_{service}_service_db?sslmode=disable" up

migrate-down:
	@migrate -path ./migrations -database "postgres://user:password@localhost:{DB_PORT}/barrowspire_{service}_service_db?sslmode=disable" down
...
5. 確認 Consul 註冊：訪問 `http://localhost:8623` 查看服務列表
