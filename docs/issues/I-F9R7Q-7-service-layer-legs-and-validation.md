...
status: open
implements: FS-F9R7Q
blocked_by: [I-F9R7Q-1, I-F9R7Q-6]
labels: [blocked]
title: "FS-F9R7Q slice 7: service layer — leg construction, sum-to-zero, reason/reference validation"
---
Implements FS-F9R7Q §Requirements 1-8, §Edge States
...
(ADR-0011 — the input type I-F9R7Q-1 defines, not a proto message).
...
`CREDIT` to positive (§Req 7). FS-F9R7Q places this in the service layer, so **the database does
...
I-F9R7Q-1 (interfaces, sentinels, open question 3), I-F9R7Q-6 (repository bodies to persist through).

## Spec Reference

FS-F9R7Q §Requirements 1–5 (what is and is not an entry), 6–8 (sign, sum-to-zero, currency),
...
Service-layer tests are hand-written here, not in I-F9R7Q-9. That slice provides the harness and
fixtures only.
