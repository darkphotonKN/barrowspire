...
status: done
implements: FS-SWMNW
blocked_by: [I-SWMNW-1, I-SWMNW-2]
labels: [blocked]
title: "FS-SWMNW slice 3: landing page — rarity ramp, the starfield decision, lore copy"
---
Implements FS-SWMNW §Requirements 15-17, 20, 23, 35-36
...
- Colours come from the `--rarity-*` tokens (I-SWMNW-2), **not** from a component-local object.
- Every badge keeps its text label — rarity is never signalled by hue alone (§R17). Already true
  in `RarityBadge`; keep it true.
- **`Legendary` is amber-bright**, which reintroduces amber wherever legendary relics grid. I-SWMNW-1
  documents this as an explicit exception to the one-torch rule. Implement against that exception;
  if I-SWMNW-1 did not settle it, stop and settle it there rather than deciding here.
...
I-SWMNW-1 (the Legendary exception, and the amber-permitted surface list), I-SWMNW-2 (the `--rarity-*`
tokens and the material primitives this page consumes).

## Spec Reference

FS-SWMNW §Requirements 15-17, 20, 23, 35-36; §User Stories 1, 2, 3, 5, 10, 11, 12, 16;
