...
labels: []
title: "FS-F9R7Q slice 5: Temporal worker, task queue, activity registration, retry policy"
---
Implements FS-F9R7Q §Requirements 16, 19, §API surface
...
> write-path plumbing, for the mechanism that replaced it. **Proto generation moved to I-F9R7Q-10**
> (which authors the read RPCs) and **gRPC registration moved to I-F9R7Q-12** (the first read
...
I-F9R7Q-2 boots for the read path.

**2. Activity registration.** `AppendLedgerTx` registered against the input/output types I-F9R7Q-1
defined, calling into the service layer I-F9R7Q-7 builds. **In-process** — there is no gRPC hop, no
...
Declare the non-retryable set explicitly, from FS-F9R7Q §API surface's write-path error table —
...
saga (FS-F9R7Q §Out of Scope), and **its absence does not block this slice**: the SDK's activity
...
events this service consumes — `internal/ledger/amqp_consumer.go` is the seat I-F9R7Q-2 was told to
...
I-F9R7Q-1 — the activity's input/output types and the sentinel set are authored there.

## Spec Reference

FS-F9R7Q §Requirements 16 (identity from execution context), 19 (write path is a Temporal
...
Plumbing plus one real decision. Leg validation and sum-to-zero are I-F9R7Q-7's — this slice must
