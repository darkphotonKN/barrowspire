...
status: done
implements: FS-F9R7Q
blocked_by: [I-F9R7Q-1]
labels: []
title: "FS-F9R7Q slice 10: read contracts — proto RPCs, query-object read interfaces, cursor encoding"
---
Implements FS-F9R7Q §API surface, §Requirements 20-23, 31

**Author: human** — do NOT hand this to `/develop`. Same reason as I-F9R7Q-1: this is the design
surface the read path is written against, and every slice after it is mechanical once the shapes
exist.

> **Extends the FS-F9R7Q chain post-amendment.** Slices 1–9 (I-F9R7Q-1 … I-F9R7Q-9) were cut before
> FS-F9R7Q gained its read path; this slice and the four after it cover what the amendment added.
>
> **This slice also absorbed proto generation from I-F9R7Q-5.** ADR-0011 made the write path a
> Temporal activity, so `AppendLedgerTx` left the proto entirely and I-F9R7Q-5 became the worker
...
**Run generation here**, absorbed from I-F9R7Q-5: regenerate `ledger.pb.go` and `ledger_grpc.pb.go`.
**Generated Go is never hand-edited** (root CLAUDE.md) — if the output is wrong, fix the `.proto`
and regenerate. Registration and handler arms are I-F9R7Q-12's, not this slice's.

**2. The read interfaces — query objects, not repository methods.** Reads do not hang off the
repository: it carries exactly one method, `Append` (§Req 9, I-F9R7Q-1). A repository provides
...
there being no such method, the same way I-F9R7Q-1 enforces no-update, no-delete, and no-read.

The DTOs are read models and stay distinct from the gateway's transport types (`Entry`,
`EntryPage`, `Transaction`, `Leg`), which are named in FS-F9R7Q §API surface's transport-type
...
by I-F9R7Q-4. The read path's shapes are the DTOs — `TransactionDetails` + `LegDetail` from
...
> bounded row set or a second query is **I-F9R7Q-13's call**; this slice only has to leave the
...
**`created_at` is on `ledger_entries`, duplicated from the parent on purpose** (FS-F9R7Q §Data
...
the migration's `CHECK`. FS-F9R7Q §Requirements 5a and §Open questions 3 record both the set and
...
I-F9R7Q-1 — the transaction-boundary pattern and the repository interface it lives on.

## Spec Reference

FS-F9R7Q §API surface (both read operations, the field tables, the error rows), §Requirements 20
