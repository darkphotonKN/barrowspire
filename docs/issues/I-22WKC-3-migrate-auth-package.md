...
title: "FS-22WKC slice 3: migrate the `auth` gateway package (28 writes)"
---
...
Delete this package's local `status.FromError` switch — one of the six duplicates FS-22WKC
§Requirements 6 exists to collapse. Its mapping logic was already collected into the seam in
I-22WKC-1; deleting it here is what makes that collection real rather than additive.
...
FS-22WKC §Requirements 4–9, 11, 12, §API surface (code→status mapping), §Edge States
