...
title: "FS-29KSH slice 1: route client messages from server-held session state"
---
Implements FS-29KSH §Requirements 16-17, §WebSocket message surface
...
FS-29KSH §Requirements 16-17; §WebSocket message surface (inbound table — `session_id` removed
from `move`, `interact`, `attack`, `equip`/`unequip`, `cast_skill`).

Rationale and rejected alternatives: FS-29KSH §Design decisions D9, §Rejected alternatives
