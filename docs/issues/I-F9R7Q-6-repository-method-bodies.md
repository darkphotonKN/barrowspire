...
status: open
implements: FS-F9R7Q
blocked_by: [I-F9R7Q-1, I-F9R7Q-3, I-F9R7Q-4]
labels: [blocked]
title: "FS-F9R7Q slice 6: repository method bodies — atomic multi-leg append, idempotent"
---
Implements FS-F9R7Q §Requirements 9-13, §Data model

**Author: agent**

> **Reassigned from human to agent.** The banner was set before I-F9R7Q-1 landed. I-F9R7Q-1 has since
...
> decided in I-F9R7Q-1 and **expressed in the interface**. Write these bodies against the pattern
...
> to I-F9R7Q-1 and change deliberately there — not a reason to reshape the interface mid-body.
...
Check I-F9R7Q-1's answer to **open question 1** before writing the conflict path: if the no-op was
...
I-F9R7Q-1 (interfaces + the transaction-boundary pattern), I-F9R7Q-3 (the table), I-F9R7Q-4 (scan
targets).

## Spec Reference

FS-F9R7Q §Requirements 9 (append-only), 10 (DB-set `created_at`), 11–13 (caller-supplied
...
Persistence only. Sum-to-zero validation is slice 7 (I-F9R7Q-7) — do not enforce it here.
...
expressed in the repository. If I-F9R7Q-1 resolved open question 1 by making the no-op conditional
...
test database and a way to race two identical posts — and I-F9R7Q-9 (the harness) is blocked by
*this* slice, so it is not available yet. Stand up whatever minimal setup these tests need
inline; I-F9R7Q-9 generalises it afterwards rather than the other way round. Do not weaken the
