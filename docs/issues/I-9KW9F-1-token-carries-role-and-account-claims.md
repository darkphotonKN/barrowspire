...
title: "FS-9KW9F slice 1: the access token carries role and account_id"
---
Implements FS-9KW9F §Requirements 1-10, §Edge States
...
populates `members.account_id` until I-9KW9F-4 closes the loop, and no member has ever had a wallet
...
> explicitly out of scope (FS-9KW9F §Out of Scope). The struct lands; the reader does not change.
...
> implementation of a capability on the roadmap (FS-9KW9F §Out of Scope).
...
FS-9KW9F §Requirements 1-6 (the claims), 7-8 (the minter), 9-10 (the column); §Edge States rows
