...
title: "FS-22WKC slice 1: error vocabulary + the single mapping seam, proven on `example`"
---
...
- `common/errcode` **already exists** (seeded by setup) with the six generic codes:
  ...
  `INTERNAL_ERROR`. Verify it against FS-22WKC §Requirements 1 rather than rewriting it. Do
...
precedence, per FS-22WKC §Requirements 5:
...
| downstream `Unavailable` | `503 · SERVICE_UNAVAILABLE` *(amended during I-22WKC-3)* |
...
The seam must be callable **from middleware, not just handlers** — I-22WKC-2 depends on this, and
...
FS-22WKC §Requirements 1–10 (vocabulary, seam, body shape, leak prevention), §API surface
...
Land on `feat/fs-0001-error-contract`, **not** main. See I-22WKC-8 — the client break must happen
