...
status: done
implements: FS-SWMNW
blocked_by: [I-SWMNW-1]
labels: [blocked]
title: "FS-SWMNW slice 2: theme foundation — tokens, fonts, page ground, material primitives, site chrome"
---
Implements FS-SWMNW §Requirements 7-15, 18-19, 21, 24, 25-29, 38

The foundation every view slice consumes, plus the chrome that proves it renders. Mechanical once
I-SWMNW-1 has authored the values — this slice reads them, it does not invent them.
...
- Reconcile `src/utils/theme.ts` (`BARROW` / `BARROW_HEX`) with the CSS custom properties. They
  ...
  one and not the other is a defect**, and FS-W6BP1 inherits whatever this slice leaves.
...
- Base and card surfaces take the lifted values from I-SWMNW-1.
...
- Blackletter on the logo only, above I-SWMNW-1's size bound.
...
- [ ] Base and card surfaces use I-SWMNW-1's lifted values
...
I-SWMNW-1 — this slice reads the values that slice authors. Starting before it means inventing
values the SSOT will contradict.

## Spec Reference

FS-SWMNW §Requirements 7-15, 18-19, 21, 24 (NotificationBell), 25-29, 38; §User Stories 4, 6, 8, 9,
