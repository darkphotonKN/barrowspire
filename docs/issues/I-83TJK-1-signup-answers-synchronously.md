...
title: "FS-83TJK slice 1: signup answers synchronously — 201 + member, 409 on duplicate, command path deleted"
---
Implements FS-83TJK §Requirements 1–10, §API surface, §Edge States

**Author: agent**

> **Renumbered I-F9R7Q-16 → I-83TJK-1.** These ids were allocated against a stale base whose highest
> issue was I-W6BP1-5; rebasing onto `main` brought in the ledger read slices, which already held
> I-F9R7Q-16 and I-F9R7Q-17. Commits landed before the collision was spotted still say "I-F9R7Q-16" —
...
> **I-83TJK-2 is reduced to removing `check-email` and regenerating.** `check-email` is NOT removed
> here.
...
- Response becomes **`201` + the bare member body**. The `acceptedEnvelope` shape and
  ...
  envelope: FS-83TJK §Requirements 2 makes signup the first operation on this surface to shed it.
...
today and wallet-service will under FS-9KW9F. This slice changes how signup is *invoked*, never
what it *announces*.
...
4. **Every entry must carry `FS-83TJK` and the reason.** An entry with no reason is a bug.
...
Leave the three `- [ ] … → FS-83TJK` lines unchecked; they flip when FS-83TJK ships (I-83TJK-2).
...
FS-83TJK §Requirements 1–10 (the signup call; removing the command path), §API surface (the
