...
title: "FS-F9R7Q slice 4: LedgerEntry struct, sqlx tags, scan targets"
---
Implements FS-F9R7Q §Data model, §Requirements 6, 8, 15
...
- `direction` is a closed set. Whether it is a `string` or a domain value type follows slice 1's
  answer to **open question 3** — check I-F9R7Q-1 before choosing.
...
- If slice 1 resolved **open question 2** with a `ledger_transactions` table, the
  ...
  onto a second one. Check I-F9R7Q-1's recorded answer first.
...
I-F9R7Q-1 — column names, the direction representation, and the open-question-2 table split are
all decided there.

## Spec Reference

FS-F9R7Q §Data model, §Requirements 6 (amount), 8 (currency per leg), 15 (`account_id` is a soft
reference). Governed by ADR-0008.

## Notes

Data shape only — no queries, no transaction handling. Those are slice 6 (I-F9R7Q-6).
