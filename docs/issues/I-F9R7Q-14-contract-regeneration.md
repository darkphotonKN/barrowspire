...
status: open
implements: FS-F9R7Q
blocked_by: [I-F9R7Q-12, I-F9R7Q-13]
labels: [blocked]
title: "FS-F9R7Q slice 14: contract regeneration — openapi.yaml and the generated client"
---
Implements FS-F9R7Q §API surface, §Requirements 32

**Author: agent**

> **Extends the FS-F9R7Q chain post-amendment.** FS-F9R7Q originally asserted `openapi.yaml` and
...
- **Spectral lint** — the `ledger` tag must already be declared globally (I-F9R7Q-11). An operation
...
- Schema component names come from the transport types in FS-F9R7Q §API surface's transport-type
...
I-F9R7Q-12 and I-F9R7Q-13 — both operations must exist as typed handlers before the document derived from
them is meaningful.

## Spec Reference

FS-F9R7Q §Requirements 32 (generated, never hand-written), §API surface (the shapes the document
