...
blocked_by: []
labels: []
title: "FS-F9R7Q slice 1: contracts — proto, interfaces, DDL, transaction-boundary pattern, sentinels"
---

Implements FS-F9R7Q §Requirements, §API surface, §Data model, §Open questions
...
**1. The activity contract** — Go input/output types for `AppendLedgerTx`, per FS-F9R7Q
...
carries read RPCs only, authored in I-F9R7Q-10.
...
Reads are query objects and belong to I-F9R7Q-10: they hold `*sqlx.DB` directly, return DTOs, bypass
...
> **Ordering constraint — do not reschedule.** Slice 6 (I-F9R7Q-6) cannot start until the
...
**4. Migration DDL** — per FS-F9R7Q §Data model. Decide the statements; slice 3 writes the
files. Includes `DROP TABLE ledgers` (§Req 17).

**5. The sentinel error set.** FS-F9R7Q §API surface's error table is the target vocabulary —
...
non-retryable on the activity's retry policy (I-F9R7Q-5), and the read path maps its own errors to
gRPC codes (I-F9R7Q-8). **The write-path classifier is `ledger.IsNonRetryable`, declared beside the
...
FS-F9R7Q §Open questions now carries all three as decisions with their reasoning. This slice
...
**What this slice still owes the DDL:** FS-F9R7Q §Data model is now the two-table shape and is the
...
- [ ] Sentinel error set covers every row of FS-F9R7Q §API surface's error table, and each row is
...
- [ ] The retryable/non-retryable classification is expressed as `ledger.IsNonRetryable` beside the
      sentinels, so I-F9R7Q-5's retry policy has one place to read it from
- [ ] The migration matches FS-F9R7Q §Data model — `created_at` on `ledger_entries`, the
...
None. This slice blocks all of I-F9R7Q-2 … I-F9R7Q-9.

## Spec Reference

FS-F9R7Q §Requirements 6–13 (invariants, idempotency), 15–16 (boundaries), 17–18 (scaffold
