...
status: done
implements: FS-83TJK
blocked_by: [I-83TJK-1]
labels: [ready-for-agent]
title: "FS-83TJK slice 2: remove the check-email endpoint"
---
Implements FS-83TJK §Requirements 11-12, 15, §API surface

**Author: agent**

> **Renumbered I-F9R7Q-17 → I-83TJK-2.** These ids were allocated against a stale base whose highest
> issue was I-W6BP1-5; rebasing onto `main` brought in the ledger read slices, which already held
> I-F9R7Q-16 and I-F9R7Q-17. Commits landed before the collision was spotted still say "I-F9R7Q-17" —
...
> **Scope reduced.** I-83TJK-1 dropped the response envelope, which made the polling loop
...
call site — I-83TJK-1 made `409` reachable, so *"An account with that email already exists."*
renders for the first time. This is the bug FS-83TJK exists to fix: today a duplicate email
polls for fifteen seconds and reports slowness.

Do not restyle the form. FS-SWMNW owns its presentation.
...
published contract is a separate decision (FS-83TJK §Req 12, §Out of Scope). Leave the RPC, its
...
`.oasdiff-ignore` entries the same way I-83TJK-1 did: run the diff, read the two indented lines,
concatenate method + path + message, lowercase, one line per triple, each citing **FS-83TJK**
and the reason. A guessed line silently fails to suppress and leaves the gate red with no
explanation.

### 4. Close out FS-83TJK
...
So: leave the checkboxes `[ ]` and FS-83TJK at `work-order`, and run `/spec-update` once this
lands on `main`.

## Acceptance Criteria

- [x] *(landed in I-83TJK-1)* `register/page.tsx` contains no `setInterval`, no `pollingRef`, no `isPolling`, and no
      `maxAttempts`
- [x] *(landed in I-83TJK-1)* A successful registration navigates to `/login` on the signup response, not on a poll
- [x] *(landed in I-83TJK-1)* A duplicate-email registration renders "An account with that email already exists."
- [x] *(landed in I-83TJK-1)* A `422` renders the validation copy; a `503` renders the unavailable copy
...
- [x] `.oasdiff-ignore` entries were copied from real diff output and each cites FS-83TJK
...
- [ ] The three FS-83TJK thin lines are checked and FS-83TJK is `Status: shipped` — **deferred to `/spec-update` on merge**, per one-writer-per-state in `docs/specs/README.md`. Not a gap in the work; the code is complete.
- [x] Client typecheck/lint and the existing suites pass

## Blocked By

I-83TJK-1 — the client needs the regenerated `201` signup operation, and `check-email` cannot be
removed while a polling client still calls it.

## Spec Reference

FS-83TJK §Requirements 11–12 (removing check-email), 13–14 (the client), 15 (regeneration);
