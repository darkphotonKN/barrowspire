...
labels: [blocked]
title: "FS-29KSH slice 4: the hub is a safe zone"
---
Implements FS-29KSH §Requirements 6
...
`MovementSystem` (FS-29KSH §Requirements 5), so the rejection must be placed where it actually
...
divergence recorded in `game-service/SPECIFICATION.md` and FS-29KSH §Out of Scope explicitly
...
I-29KSH-2 — the hub must exist to be a safe zone.

## Spec Reference

FS-29KSH §Requirements 6; thin line `game-service/SPECIFICATION.md` "### Hub world" → "Hub is a
