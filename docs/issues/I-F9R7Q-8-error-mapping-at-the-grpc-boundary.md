...
status: open
implements: FS-F9R7Q
blocked_by: [I-F9R7Q-1, I-F9R7Q-2]
labels: []
title: "FS-F9R7Q slice 8: read-path error mapping at the gRPC boundary — sentinels to status codes"
---
Implements FS-F9R7Q §API surface

**Author: human** — do NOT hand this to `/develop`.

## What to Build

Fill `mapError` in `internal/ledger/grpc/handler.go`. **I-F9R7Q-2 must have left it empty** — if it
...
**I-F9R7Q-5's** job, in I-F9R7Q-5's file. Do not map write-path sentinels here; there is no caller for
them.

Map the sentinel set to gRPC status codes for the two read operations, per FS-F9R7Q §API surface's
read-path rows:
...
> for which transaction ids are real. I-F9R7Q-12 owns the enforcement; this function must not offer a
...
and they are classified, on the activity's retry policy in I-F9R7Q-5.

## Acceptance Criteria

- [ ] `mapError` was empty at the start of this slice — verified, not assumed
- [ ] Every row of FS-F9R7Q §API surface's **read-path** error set has exactly one mapping
- [ ] No mapping exists for `ErrConcurrentModification` or `ErrMaxRetries`
- [ ] No mapping exists for any write-path sentinel — those are classified in I-F9R7Q-5
...
- [ ] The TODO left by I-F9R7Q-2 is removed
- [ ] Tests assert the code for each mapped sentinel
- [ ] `make lint && make test` green

## Blocked By

I-F9R7Q-1 (the sentinel set), I-F9R7Q-2 (the empty stub this fills).

## Spec Reference

FS-F9R7Q §API surface (read-path error rows), §Requirements 17 (OCC removed), 24–27
...
I-F9R7Q-12 extends this function with nothing new — its rows are already here. If I-F9R7Q-12 finds itself
