...
title: "FS-F9R7Q slice 3: migration files — create ledger_entries, drop ledgers"
---
Implements FS-F9R7Q §Data model, §Requirements 6, 9-10, 17

**Author: agent**

> **CLOSED — satisfied by I-F9R7Q-1, not by a separate slice.** I-F9R7Q-1 landed the DDL directly as
...
**Up** — create `ledger_entries` per FS-F9R7Q §Data model: the `amount_positive` and
...
`ledger_transactions` table, this migration creates it too** — check I-F9R7Q-1's recorded answers
...
I-F9R7Q-1 — the DDL is decided there, including whether open question 2 adds a second table.

## Spec Reference

FS-F9R7Q §Data model, §Requirements 6 (amount > 0), 9–10 (append-only, DB-set `created_at`),
