...
status: done
implements: FS-SWMNW
blocked_by: [I-SWMNW-2]
labels: [blocked]
title: "FS-SWMNW slice 4: reskin the remaining views — auth, profile, leaderboard"
---
Implements FS-SWMNW §Requirements 15, 20, 24, 35-36

The three views with no judgment calls left in them. One slice because it is the same mechanical
pass three times: apply I-SWMNW-2's vocabulary, purge the enumerated hexes, fix the copy.

The `globals.css` class families are cleanly separated by prefix, so these do not collide with
each other or with I-SWMNW-3.
...
I-SWMNW-2 — consumes its tokens, fonts and material primitives.

Independent of I-SWMNW-3 and I-SWMNW-5: separate class prefixes, separate files. The three may run
concurrently with those slices.

## Spec Reference

FS-SWMNW §Requirements 15, 20, 24 (leaderboard `styled-jsx`), 35-36; §User Stories 7, 15, 16;
