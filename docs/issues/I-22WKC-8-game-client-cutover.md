...
status: in-progress
implements: FS-22WKC
blocked_by: [I-22WKC-7]
labels: [ready-for-agent]
title: "FS-22WKC slice 8: game-client reads `code` — the cutover, and the single merge to main"
---
...
`code`, display `detail` (FS-22WKC §Requirements 15).
...
Slices I-22WKC-1 through I-22WKC-7 land on `feat/fs-0001-error-contract`. **This slice's merge is the
one that reaches main.** FS-22WKC locked "break the client once" — but merging each package slice
...
- [ ] Every error class in FS-22WKC §API surface exercised **before and after** against a running gateway, pairs recorded in the PR
...
I-22WKC-7 — and transitively every migration slice. The client cuts over only once the server
speaks one shape everywhere.

## Spec Reference

FS-22WKC §Requirements 15, §API surface (member table — `code` is the switch key, `detail` is
