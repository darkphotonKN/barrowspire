...
title: "FS-NTPW2 slice 1: Huma wiring + the member group — the yaml is born"
---
Implements FS-NTPW2 §Requirements 1-4, 8-14, §API surface
...
- **Identity bridge**: typed handlers read the same `userId`/`userIdStr` that `AuthMiddleware`
  sets. One source of caller identity, never the request body (FS-NTPW2 §Req 3).
...
None. This slice blocks I-NTPW2-2, I-NTPW2-3, I-NTPW2-4.

## Spec Reference

FS-NTPW2 §Requirements 1-4 (mounting, identity), 8-9 (errors), 10-11 (the two deliberate
