...
status: done
implements: FS-W6BP1
blocked_by: [I-W6BP1-1]
labels: [blocked]
title: "FS-W6BP1 slice 2: the semantic canvas palette, the font constant, and the Cinzel regression"
---
Implements FS-W6BP1 §Requirements 6-13

The foundation the recolour consumes. Mechanical once I-W6BP1-1 has authored the channels — this
...
- Channel names should map onto I-W6BP1-1's accent rule where they overlap, so `palette.interactable`
  and "amber means actionable" are visibly the same decision.

### Reconcile the base (§R8)

The canvas world background is `0x0d0b0a` — the value FS-SWMNW lifted to `0x1c1613`. Canvas and
DOM currently disagree about what "dark" means. `PhaserGame.tsx`'s `backgroundColor` was already
moved onto `BARROW.umber` in FS-SWMNW; the in-scene fills were not.

### Typography (§R10-R13)

- **Fix the regression FS-SWMNW introduced.** Three text objects declare
...
- Blackletter only on the surfaces I-W6BP1-1 permits, never below 28px.
...
I-W6BP1-1 — the channel names and the typography bound come from that amendment.

## Spec Reference

FS-W6BP1 §Requirements 6-13 (§B. The canvas palette module, §C. Typography); §User Stories 7, 9,
