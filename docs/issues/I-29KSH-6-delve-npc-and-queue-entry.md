...
status: done
implements: FS-29KSH
blocked_by: [I-29KSH-5]
labels: [blocked]
title: "FS-29KSH slice 6: delve NPC and queue entry"
---
Implements FS-29KSH §Requirements 9, 24-28
...
- **Dialogue box with options.** Interacting opens a box; the queue is joined only on the
  ...
  branching, no per-player memory, no dialogue data format (FS-29KSH §Out of Scope).
...
deliberately still cannot cancel. This is recorded in FS-29KSH §Accepted consequences — it is
chosen, not missed.
...
- [x] Two players queueing in the same tick both queue, and reaching `matchSize` matches them
      into the same run.
- [x] **Revised, as in I-29KSH-1 and I-29KSH-5:** no new test failures, no new lint findings versus
...
Unlike I-29KSH-5, this slice was run. Three defects surfaced that the suite had not:
...
I-29KSH-5 — queuing must lead somewhere, so the world switch has to work first.

## Spec Reference

FS-29KSH §Requirements 9, 24-28; §Edge States (Empty — "Nobody else queued: the player waits
