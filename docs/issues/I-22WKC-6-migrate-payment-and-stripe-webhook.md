...
title: "FS-22WKC slice 6: migrate `payment` including POST /webhook/stripe (10 writes)"
---
Implements FS-22WKC §Requirements, §API surface, §Edge States

## What to Build

Route all 10 error writes in `api-gateway/internal/gateway/payment/handler.go` through the seam
and delete the last of the six duplicated `status.FromError` switches (FS-22WKC §Requirements 6).
...
- Every webhook failure status must be **compared before/after** against current behavior — status parity is the contract with Stripe, and no automated gate can check it (see I-22WKC-8: there is no `openapi.yaml` to diff against)
...
shape — FS-22WKC §Summary's "nothing about what the gateway does changes" holds for the body and
not for the status. Confirmed on `example` in I-22WKC-1, where an unhandled `AlreadyExists` moved
...
FS-22WKC §Requirements 4–9, 11 (Stripe migrated, no exemption), 12, §Edge States (webhook with
