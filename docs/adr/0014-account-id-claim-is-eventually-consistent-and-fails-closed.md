...
says otherwise
Realized by: FS-9KW9F (draft) — `docs/specs/9KW9F-account-and-role-token-claims.md`

## Context

FS-F9R7Q §Requirement 27 designed the ledger read path around a member's `account_id` arriving
...
every balance and hold operation; the ledger holds no account records at all (FS-F9R7Q §Req 15).
...
4. When `members.account_id` is NULL, the minter **omits the claim entirely** rather than
   ...
   bug at the boundary.
5. A token without the claim is rejected — `401 · UNAUTHENTICATED` per FS-F9R7Q — never
...
- **Resolve at the gateway per request.** Reintroduces exactly the hop FS-F9R7Q §Req 27 removed.
- **Derive the id deterministically** (`accounts.id = member_id`, or `UUIDv5(ns, member_id)`).
  ...
  an artifact of one-account-per-member, the same assumption FS-F9R7Q §Req 27 already flags as a
  revisit trigger.
...
- The login path gains no new synchronous dependency. Wallet can be down and members still
  ...
  wrong answer.
- The ledger read path stays at one hop, which is what FS-F9R7Q §Req 27 bought and this decision
  preserves.
...
- **The always-present-and-immutable property FS-F9R7Q §Req 27 assumed is now false.** Anything
  written against that assumption must be re-read. The claim is eventually present, not always
  present.
- **Existing members never get a claim.** No backfill is in scope (FS-9KW9F, Decision 3), and
...
trigger FS-F9R7Q §Req 8 and §Req 27 are already waiting on.
