# FS-NTPW2: Gateway HTTP surface serialized
...
10. **Shape validation fails 422.** ADR-0001 §7 reserves 422 for shape failures decided at the
    boundary from a typed signature; FS-22WKC recorded that 422 "does not appear ... until Huma
...
16. **Each group ships with a recorded before/after comparison.** Every endpoint in the group is
    ...
    byte-compatible, with error bodies excepted (changed in FS-22WKC, already shipped) and the
...
- **A downstream returns an unmapped gRPC code** → unchanged from FS-22WKC: `500 INTERNAL_ERROR`
...
Error responses are already pinned by [FS-22WKC §API surface](22WKC-uniform-error-contract.md) and
