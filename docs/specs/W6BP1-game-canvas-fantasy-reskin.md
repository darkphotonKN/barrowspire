# FS-W6BP1: Fantasy-medieval presentation in the game canvas

> Status: work-order · SPECIFICATION.md: `game-client/SPECIFICATION.md` "## Capabilities" → "### Presentation" → "Fantasy-medieval presentation in the game canvas" → this FS · Art SSOT: [`game-client/docs/design-guideline.md`](../../game-client/docs/design-guideline.md) Part I · Vocabulary: [`game-client/CONTEXT.md`](../../game-client/CONTEXT.md) · Sibling: [FS-SWMNW](SWMNW-web-platform-fantasy-reskin.md) (web platform, shipped) · Related ADRs: [ADR-0013](../adr/0013-client-styling-is-token-only-and-the-fence-must-be-watched-to-fail.md) (token-only styling, fence-enforced)

## Summary

The Phaser canvas is the half of the client FS-SWMNW did not touch. It still draws in a cold
sci-fi blue-grey, renders 56 of its 59 text objects in Phaser's default sans, opens on an
unthemed stock loading bar, and is lit from nowhere. It also now disagrees with the DOM: FS-SWMNW
...
Same shape as FS-SWMNW §A: the guideline holds the values, this spec states the properties the
...
2. **Part I typography is amended** to blackletter display + old-style serif body, matching
   FS-SWMNW, and **recorded as a deliberate deviation** from Part I's current "medieval
...
5. **The amendment names which Part I rules remain pending on assets** — nearest-neighbour
   ...
   `image-rendering: pixelated`. FS-W6BP1 does not discharge them, and the guideline must not read
   as though it did.
...
8. The canvas base agrees with the DOM. The world background is currently `0x0d0b0a` — the base
   FS-SWMNW lifted to `0x1c1613`. Canvas and DOM must not disagree about what "dark" means.
...
10. **Fix the regression FS-SWMNW introduced.** Three text objects declare
...
25. The ADR-0013 fence widens to cover the canvas layer. FS-SWMNW scoped it to the DOM surface and
...
- **Blackletter in-game is riskier than on the web.** HUD text is small and read under pressure.
  ...
  title proves hard to read at size, the FS-SWMNW fallback applies here too (Grenze, MedievalSharp,
  IM Fell).
...
- **The web platform** — [FS-SWMNW](SWMNW-web-platform-fantasy-reskin.md), shipped.
