...
6. **`member.signedup` is still published**, via auth-service's existing transactional outbox
   ...
   *announces* — notification-service and, under FS-9KW9F, wallet-service both continue to
...
11. As **wallet-service under FS-9KW9F**, I want the member to exist before I am told about them,
    so that my account creation is one bounded hop rather than a race against an unbounded one.
12. As **a member registering under FS-9KW9F**, I want the signup-to-claim path to be as short as
...
- [ ] The three FS-83TJK thin lines are checked
...
| Outbox worker stalled after a successful signup | Signup succeeded and the member exists; only downstream consumers (notification, and wallet under FS-9KW9F) are delayed. Pre-existing outbox behavior, unchanged. |
...
- **Auto-login on signup.** Considered; signup returns a member, not tokens (requirement 5).
  Notably, under FS-9KW9F an auto-login token would be *guaranteed* to lack `account_id`, since
...
- **Fixing auth-service's AMQP consumer to nack on error.** This feature deletes the
  `member.create` branch; the consumer's other handling is untouched. FS-9KW9F requirement 17
  binds only its own new consumers.
- **The token claims themselves** — FS-9KW9F. This feature's only relationship to it is
  sequencing: collapsing signup to one synchronous hop bounds FS-9KW9F's `account_id` window.
...
- **The register form's visual design** — FS-SWMNW owns presentation.
