# FS-9KW9F: Account and role claims on the access token

> Status: shipped · SPECIFICATION.md: `game-server/auth-service/SPECIFICATION.md` "## Capabilities" → "### Tokens" → "Mint account and role claims into the access token" and "### Accounts" → "Record a member's wallet account id"; `game-server/wallet-service/SPECIFICATION.md` "## Capabilities" → "### Account" → "Create a member's account on signup" and "Publish account creation for downstream consumers" → this FS · Related ADRs: [ADR-0014](../adr/0014-account-id-claim-is-eventually-consistent-and-fails-closed.md) (eventually-consistent claim, fails closed), [ADR-0005](../adr/0005-wallet-owns-balance-ledger-is-a-reconciliation-record.md) (wallet owns balance) · Consumer: [FS-F9R7Q](F9R7Q-append-only-gold-ledger.md) §Req 24–29 · Sibling: [FS-83TJK](83TJK-synchronous-signup.md) (synchronous signup — **sequenced before this**) · Vocabulary: [`auth-service/CONTEXT.md`](../../game-server/auth-service/CONTEXT.md), [`wallet-service/CONTEXT.md`](../../game-server/wallet-service/CONTEXT.md)

## Summary

auth-service's access token carries only `sub`, `exp`, `iat`, and `tokenType`. FS-F9R7Q built the
...
satisfied, which is what blocks I-F9R7Q-12 `getTransaction` and I-F9R7Q-13 `listEntries`.
...
22. **No backfill.** Members who predate this feature have `account_id` NULL and no claim, and
    ...
    pre-launch (ADR-0014).
23. **FS-F9R7Q §Req 29 is amended from `member | admin` to `player | admin`**, together with its
    edge-state rows and the `role=member` references in §Req 25. Done as part of this work
    order; FS-F9R7Q is `work-order`, not `shipped`, so the write-once rule does not bind.
24. **This feature assumes signup is synchronous ([FS-83TJK](83TJK-synchronous-signup.md)), and is
    ...
    bound on the total delay. FS-83TJK collapses the first hop to a synchronous call, leaving
    ...
    **The dependency is only on ordering, not on design.** `member.signedup` is published from
    inside `CreateMember` via the outbox either way (FS-83TJK §Req 6), so requirement 11's
    consumer is unaffected by which transport invoked signup. Shipping this feature first is
    possible; the window is simply unbounded until FS-83TJK lands.
...
6. As **an admin**, I want my `admin` role to arrive as a verified claim, so that FS-F9R7Q's
...
7. As **ledger-service**, I want the caller's account id to arrive already verified, so that I
   can scope a query by account without holding any account records (FS-F9R7Q §Req 15).
...
24. As **whoever builds a refresh-redemption endpoint next**, I want refresh tokens to carry no
    ...
    seven-day-old copy.
25. As **a reader of FS-F9R7Q**, I want §Req 29's placeholder role vocabulary corrected to what
...
- [ ] FS-F9R7Q contains no occurrence of `role=member` or `member | admin`
...
- **The ledger read path itself** — FS-F9R7Q, I-F9R7Q-12 `getTransaction`, I-F9R7Q-13 `listEntries`. This
