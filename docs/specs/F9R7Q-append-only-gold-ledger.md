# FS-F9R7Q: Append-only gold ledger
...
27. **A member's `account_id` arrives as a verified token claim.** There is no resolution step and
    ...
    that reads the claim; **minting it landed in [FS-9KW9F](9KW9F-account-and-role-token-claims.md).**
...
29. **`role` arrives as a verified claim on every access token, and a request carrying none is
    refused at the boundary.** [FS-9KW9F](9KW9F-account-and-role-token-claims.md) mints `role` off
    ...
    **There is no default role.** An earlier reading of this requirement had the boundary resolve
    ...
    > **Vocabulary amended by [FS-9KW9F](9KW9F-account-and-role-token-claims.md).** The placeholder
...
> the page's bounded row set or a second query is **I-F9R7Q-13's to settle**; it is not free either
> way.
...
- **Issuing the `role` and `account_id` claims.** This feature reads both from verified token
  ...
  [FS-9KW9F](9KW9F-account-and-role-token-claims.md)**. This feature builds only the seam that
  consumes them.
